...
- **`site.json` y `common.json` están completos en los 10 idiomas**: las 147 claves que usa el sitio resuelven en todos, sin caer al fallback. Al agregar una clave nueva hay que sumarla en los 10 para que siga así; el diff de claves contra `es/` es la forma de medirlo.
  - El texto del botón de los CTA de `/funciones` y `/casos-de-uso` (`featuresPage.cta.button`, `useCasesPage.cta.button`) es una copia de `common.chrome.ctaStart`, no una traducción aparte: es el mismo botón que el del header y conviene que digan lo mismo en la misma página.
