...
const SiteFooter = ({ external }: Props) => {
      ...
      <div className="relative max-w-6xl mx-auto px-6 md:px-8">
        <div className="grid grid-cols-1 gap-10 text-center md:grid-cols-[2fr_1fr_1fr] md:gap-8 md:text-left">
          <div className="flex flex-col items-center md:block md:items-stretch">
            <SiteLink external={external} to="/" className="flex items-center w-fit">
              <img src={logo} alt={t("chrome.logoAlt")} className="h-8 w-auto object-contain" />
            </SiteLink>
            <p className="mt-3 max-w-xs text-sm text-muted-foreground">{t("chrome.footerNav.tagline")}</p>
          </div>

          <div className="flex flex-col items-center md:block md:items-stretch">
            <p className="text-[11px] uppercase tracking-[0.25em] font-bold text-muted-foreground">
              {t("chrome.footerNav.linksTitle")}
            </p>
            <nav className="mt-4 flex flex-col items-center gap-3 md:items-stretch">
              {FOOTER_LINKS.map(({ to, labelKey }) => (
                <SiteLink key={to} external={external} to={to} className="w-fit text-sm font-medium text-foreground/80 transition-colors hover:text-primary">
                  {t(`chrome.nav.${labelKey}`)}
                </SiteLink>
              ))}
              <a href="/app/auth?mode=login" className="w-fit text-sm font-medium text-foreground/80 transition-colors hover:text-primary">
                {t("chrome.nav.login")}
              </a>
              <a href="/app/auth?mode=signup" className="w-fit text-sm font-medium text-foreground/80 transition-colors hover:text-primary">
                {t("chrome.ctaStart")}
              </a>
            </nav>
          </div>
          ...
          <div className="flex flex-col items-center md:block md:items-stretch">
            <p className="text-[11px] uppercase tracking-[0.25em] font-bold text-muted-foreground">
              {t("chrome.footerNav.socialTitle")}
            </p>
            <div className="mt-4 flex items-center justify-center gap-3 md:justify-start">
              {SOCIAL_LINKS.map(({ icon: Icon, href, ariaKey }) => (
                <a
                  ...
                  aria-label={t(`chrome.footerNav.social.${ariaKey}`)}
        ...
        <div className="mt-12 border-t border-border/50 pt-6 text-center">
          <p className="text-xs text-muted-foreground">{t("chrome.footer")}</p>
        </div>
      </div>
    </footer>
  );
