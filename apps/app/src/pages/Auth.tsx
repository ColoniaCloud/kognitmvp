...
import { useToast } from "@kognit/ui/hooks/use-toast";
...
import { Form, FormControl, FormField, FormItem, FormMessage } from "@kognit/ui/components/form";
import { SiteHeader } from "@kognit/ui/site/SiteHeader";
import { SiteFooter } from "@kognit/ui/site/SiteFooter";
...
// Reutiliza el contenido de plans.* (mismo texto que SitePricing.tsx en la
  ...
  return (
    <button
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-bold">{t(`plans.${plan}.name`)}</p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {plan === "pro" ? t("plans.pro.badge") : t(`plans.${plan}.priceNote`)}
          </p>
        </div>
        <div className="shrink-0 text-right">
          <p className="text-lg font-bold">{t(`plans.${plan}.price`)}</p>
          {plan === "pro" && <p className="text-[10px] text-muted-foreground">{t("plans.pro.priceSuffix")}</p>}
        </div>
      </div>
    </button>
  );
...
export default function Auth() {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-hero overflow-hidden flex flex-col">
      {!isStandalone && <SiteHeader external />}
      ...
      <div className="relative w-full max-w-md">
        ...
        {/* Va al sitio público, que es el otro build: con basename="/app" un <Link to="/">
            resolvería a /app/ (la app misma) en vez de a la landing. */}
        <a href="/" className="mt-5 block text-center text-xs text-muted-foreground">{t("auth.viewDemo")}</a>
      </div>
      </div>

      {!isStandalone && <SiteFooter external />}
    </div>
  );
}
