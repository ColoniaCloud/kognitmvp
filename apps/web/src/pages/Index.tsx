...
import { SiteHeader } from "@kognit/ui/site/SiteHeader";
import { SitePricing } from "@/components/site/SitePricing";
import { AppScreensCarousel } from "@/components/site/AppScreensCarousel";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@kognit/ui/components/accordion";

/**
 * `lottie-react` arrastra lottie-web (~250 KB) y acá solo anima la mascota
 * decorativa del hero, que además no se rendea hasta que llega el JSON por fetch.
 * Cargarlo aparte lo saca del camino crítico del primer pintado.
 */
const Lottie = lazy(() => import("lottie-react"));

/**
 * Las dos secciones más pesadas del landing están abajo del fold, así que no tienen
 * por qué bloquear el primer pintado:
 *  - `PrelaunchSignup` es el único que necesita el cliente de Supabase, zod y
 *    react-hook-form en el home.
 *  - `SiteFooter` es el único que usa framer-motion.
 */
const PrelaunchSignup = lazy(() =>
  import("@/components/site/PrelaunchSignup").then((m) => ({ default: m.PrelaunchSignup })));
const SiteFooter = lazy(() =>
  import("@kognit/ui/site/SiteFooter").then((m) => ({ default: m.SiteFooter })));
          ...
          <div className="relative flex w-full flex-1 flex-col gap-10 md:grid md:flex-none md:grid-cols-2 md:items-center">
            ...
            <div className="relative flex flex-1 flex-col md:block md:flex-none">
              ...
              <div className="mt-auto flex flex-nowrap items-center gap-3 pt-10 md:mt-6 md:pt-0">
                <a href="/app/auth?mode=signup" className="flex-shrink-0 bg-gradient-primary text-primary-foreground font-bold px-6 py-3 rounded-full shadow-soft text-sm">
                  {t("chrome.ctaStart")}
                </a>
